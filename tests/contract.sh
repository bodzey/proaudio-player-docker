...
test_build_contract() {
    ...
    assert_contains Dockerfile 'librespot-discovery feature "with-libmdns"'
    assert_contains Dockerfile '! grep -Fq'
    assert_not_contains Dockerfile 'cargo install spotifyd --locked --version'
...
test_s6_services() {
    ...
    assert_contains docker/s6-service-run 'wait_for_avahi'
    assert_contains docker/s6-service-run 'org.freedesktop.Avahi'
...
test_runtime_policy() {
    ...
    assert_contains docker/run-audio-buses.sh 'shutdown_requested=0'
    assert_contains docker/run-audio-buses.sh 'shutdown_requested=1'
    assert_contains docker/run-audio-buses.sh 'if ((shutdown_requested == 0)); then'
